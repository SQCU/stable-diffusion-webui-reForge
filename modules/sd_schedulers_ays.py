...
def sgm_uniform(n, sigma_min, sigma_max, inner_model, device):
 ...
 def sigmax_uniform(n, sigma_min, sigma_max, inner_model, device):
    start = inner_model.sigma_to_t(torch.tensor(sigma_max*6))
    end = inner_model.sigma_to_t(torch.tensor(sigma_min))
    sigs = [
        inner_model.t_to_sigma(ts)
        for ts in torch.linspace(start, end, n + 1)[:-1]
...
schedulers = [
    ...
    Scheduler('sigmax_uniform', 'SIGMAX6 Uniform', sigmax_uniform, need_inner_model=True, aliases=["SIGMAXUniform"]),
]
